  ...
  return (
    <div style={{ padding: '32px' }}>
      ...
      <div style={{ background: '#ffffff', borderRadius: '16px', border: '1px solid #f3f4f6', overflow: 'hidden' }}>
        ...
        {hospitals.map((h, i) => (
          <div key={h.id} style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr 2fr 1fr 1fr', padding: '20px 24px', borderBottom: i < hospitals.length - 1 ? '1px solid #f3f4f6' : 'none', alignItems: 'center', transition: 'background 0.2s' }} onMouseEnter={e => e.currentTarget.style.background = '#f3f4f6'} onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>
            <div style={{ fontWeight: '600', color: '#111827' }}>{h.name}</div>
            <div style={{ color: '#6b7280' }}>{h.doctor}</div>
            ...
            <div style={{ color: h.neca ? '#4ade80' : '#6b7280' }}>{h.neca || '미등록'}</div>
          </div>
        ))}
      </div>
