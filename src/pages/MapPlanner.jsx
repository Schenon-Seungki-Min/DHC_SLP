    ...
    <div style={{ padding: '32px' }}>
      ...
      <div style={{ marginBottom: '24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: '8px' }}>
          {['전체', '수도권', '지방'].map(f => (
            <button key={f} onClick={() => setFilter(f)} style={{ padding: '8px 16px', background: filter === f ? '#3b82f6' : 'transparent', border: '1px solid #f3f4f6', borderRadius: '8px', color: filter === f ? '#fff' : '#6b7280', cursor: 'pointer', fontSize: '14px' }}>{f}</button>
          ))}
        </div>
      </div>
        ...
        <div style={{ background: '#ffffff', borderRadius: '16px', border: '1px solid #f3f4f6', height: '600px', position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ position: 'absolute', bottom: '16px', left: '16px', background: '#f9fafb', padding: '12px 16px', borderRadius: '12px', fontSize: '12px', border: '1px solid #f3f4f6' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              <span style={{ color: '#6b7280' }}>내 리스트</span>
            </div>
          </div>
        </div>
