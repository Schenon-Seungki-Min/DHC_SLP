...
const clients = [
  { id: 1, name: '서울수면클리닉', address: '서울시 강남구 테헤란로 123', staff: '김수면', otherStaff: ['이진료', '박상담'], phone: '010-1234-5678', email: 'seoul@sleep.kr', lastVisit: '2024-12-20', neca: '2024-06-15', partners: [{name: 'A파트너', date: '2024-12-20'}, {name: 'B파트너', date: '2024-12-18'}] },
  { id: 2, name: '강남브레인의원', address: '서울시 강남구 역삼동 456', staff: '이두뇌', otherStaff: ['최신경'], phone: '010-2345-6789', email: 'brain@clinic.kr', lastVisit: '2024-12-18', neca: '2024-07-20', partners: [{name: 'A파트너', date: '2024-12-18'}] },
  { id: 3, name: '분당숙면병원', address: '경기도 성남시 분당구 정자동 789', staff: '박숙면', otherStaff: [], phone: '010-3456-7890', email: 'bundang@sleep.kr', lastVisit: '2024-12-15', neca: '2024-08-10', partners: [{name: 'C파트너', date: '2024-12-15'}, {name: 'A파트너', date: '2024-12-10'}] },
  { id: 4, name: '인천꿈의원', address: '인천시 연수구 송도동 321', staff: '정꿈나라', otherStaff: ['한밤잠', '오숙면'], phone: '010-4567-8901', email: 'dream@incheon.kr', lastVisit: '2024-12-22', neca: null, partners: [{name: 'B파트너', date: '2024-12-22'}] },
];
  ...
  return (
    <div style={{ padding: '32px' }}>
      ...
      <div style={{ background: '#ffffff', borderRadius: '16px', border: '1px solid #e5e7eb', overflow: 'hidden', boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
        ...
        {/* Data Rows */}
        {clients.map((c, i) => (
          <div key={c.id} style={{ display: 'grid', gridTemplateColumns: '1.5fr 2fr 1fr 1.2fr 1.5fr 1fr', padding: '20px 24px', borderBottom: i < clients.length - 1 ? '1px solid #f3f4f6' : 'none', alignItems: 'center', transition: 'background 0.2s' }} onMouseEnter={e => e.currentTarget.style.background = '#f3f4f6'} onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>
            <div style={{ fontWeight: '600', color: '#111827', cursor: 'pointer' }} onClick={() => setPopup({ type: 'neca', data: c })}>{c.name}</div>
            <div style={{ color: '#6b7280', fontSize: '14px', cursor: 'pointer' }} onClick={() => window.open(`https://map.kakao.com/link/search/${encodeURIComponent(c.address)}`)}>{c.address}</div>
            <div style={{ color: '#38bdf8', cursor: 'pointer' }} onClick={() => setPopup({ type: 'staff', data: c })}>{c.staff}</div>
            <div style={{ color: '#4ade80', cursor: 'pointer', fontFamily: 'monospace' }} onClick={() => setSmsPopup(c)}>{c.phone}</div>
            <div style={{ color: '#a78bfa', cursor: 'pointer', fontSize: '14px' }} onClick={() => window.location.href = `mailto:${c.email}`}>{c.email}</div>
            <div style={{ color: '#fbbf24', cursor: 'pointer' }} onClick={() => setSchedulePopup(c)}>{c.lastVisit}</div>
          </div>
        ))}
      </div>
